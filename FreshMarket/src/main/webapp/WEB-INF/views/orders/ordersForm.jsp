				...
				<div class="col-md-12 pr-2 mt-4">
					...
					<div class="card mb-5 mt-3 sub">
						<div class="row">
							...
							<div class="col-sm-4 mt-2" id="billName" value="${cartMap.memberDTO.name}">${cartMap.memberDTO.name}</div>
						</div>
						<hr class="board">
						<div class="row">
							...
							<div class="col-sm-4">${cartMap.memberDTO.email}</div>
						</div>
						<input type="hidden" id="billAddress" value="${cartMap.memberDTO.address}"/>
						<hr class="board">
						<div class="row">
...
<c:forEach var="item" items="${cartMap.listCart}" varStatus="status">
						...
						<div class="row main2 d-flex flex-row justify-content-between">
							<div class="col-sm-9 orderbox1">
								<span class="title">${status.count}.&nbsp;&nbsp;&nbsp;${item.product_name}</span>
							</div>
							<div class="col-sm-2 align-self-center orderbox2">
								<div class="row">수량 ${item.cart_product_amount}개</div>
							</div>
						</div>
</c:forEach>
					</div>
		...
		else
		{
	        ...
	        IMP.request_pay({
	            ...
	            */
	            ...
	            name: '${cartMap.memberDTO.name}',
	            //결제창에서 보여질 이름
	            amount: ${cartMap.allSum},
	            //가격 
	            buyer_email: '${cartMap.memberDTO.email}',
	            buyer_name: '${cartMap.memberDTO.name}',
	            buyer_tel: phoneHypen,
	            buyer_addr: '${cartMap.memberDTO.address}',
	            buyer_postcode: '',
	            m_redirect_url: '${contextPath}/orders/ordersProcess.do'
	        }, function (rsp) {
	            console.log(rsp);
	            if (rsp.success) {
	                var msg = '결제가 완료되었습니다.';
	                ...
	                form.setAttribute("action", "${contextPath}/orders/ordersProcess.do");
