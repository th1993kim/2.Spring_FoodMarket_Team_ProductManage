				...
				<div class="col-md-12 pr-2 mt-4">
					...
					<div class="card mb-5 mt-3 sub">
						<div class="row">
							...
							<div class="col-sm-4 mt-2" id="billName" value="${directOrderMap.memberDTO.name}">${directOrderMap.memberDTO.name}</div>
						</div>
						<hr class="board">
						<div class="row">
							...
							<div class="col-sm-4">${directOrderMap.memberDTO.email}</div>
						</div>
						<input type="hidden" id="billAddress" value="${directOrderMap.memberDTO.address}"/>
						<hr class="board">
						<div class="row">
					...
					<div class="card mb-5 mt-3 pb-3  sub">
						...
						<div class="row main2 d-flex flex-row justify-content-between">
							<div class="col-sm-9 orderbox1">
								<span class="title">1.&nbsp;&nbsp;&nbsp;${directOrderMap.cartDTO.product_name}</span>
							</div>
							<div class="col-sm-2 align-self-center orderbox2">
								<div class="row">수량 ${directOrderMap.cartDTO.cart_product_amount}개</div>
							</div>
						</div>
					</div>
					<div class="subtitle mt-4">
					...
					<div class="card mb-5 mt-3 sub">
						<div class="row">
							...
							<div class="col-sm-4 mt-2">
								<span>&#x20A9;</span> <strong class="price-value" id="sum_p_price"><fmt:formatNumber value="${directOrderMap.sumMoney}" type="number" var="sumMoney" />${sumMoney}</strong>원
							</div>
						</div>
						<hr class="board">
						<div class="row">
							...
							<div class="col-sm-4">
								<span>&#x20A9; </span> <strong class="price-value" id="fee_price"><fmt:formatNumber value="${directOrderMap.fee}" type="number" var="delivery_fee" />${delivery_fee}</strong>원
							</div>
						</div>
						<hr class="board">
						<div class="row">
							...
							<div class="col-sm-4 mb-2">
								<span>&#x20A9;</span> <strong class="price-value" id="sum_all_price"><fmt:formatNumber value="${directOrderMap.allSum}" type="number" var="allSum" />${allSum}</strong>원
							</div>
						</div>
					</div>
		...
		else
		{
	        ...
	        IMP.request_pay({
	            ...
	            */
	            ...
	            name: '${directOrderMap.memberDTO.name}',
	            //결제창에서 보여질 이름
	            amount: ${directOrderMap.allSum}, 
	            //가격 
	            buyer_email: '${directOrderMap.memberDTO.email}',
	            buyer_name: '${directOrderMap.memberDTO.name}',
	            buyer_tel: phoneHypen,
	            buyer_addr: '${directOrderMap.memberDTO.address}',
	            buyer_postcode: '',
	            ...
	            if (rsp.success) {
	                ...
	                hiddenField.setAttribute("value", ${directOrderMap.fee});
	                form.appendChild(hiddenField);

	                hiddenField = document.createElement("input");
	                hiddenField.setAttribute("type", "hidden");
	                hiddenField.setAttribute("name", "product_id");
	                hiddenField.setAttribute("value", ${directOrderMap.cartDTO.product_id});
	                form.appendChild(hiddenField);

	                var obj6 = "${directOrderMap.cartDTO.product_name}";
	                ...
	                hiddenField.setAttribute("value", ${directOrderMap.cartDTO.cart_product_price});
	                form.appendChild(hiddenField);

	                hiddenField = document.createElement("input");
	                hiddenField.setAttribute("type", "hidden");
	                hiddenField.setAttribute("name", "cart_product_amount");
	                hiddenField.setAttribute("value", ${directOrderMap.cartDTO.cart_product_amount});
	                form.appendChild(hiddenField);

	                var obj7 = "${directOrderMap.cartDTO.product_img_path}";
	                ...
	                var obj8 = "${directOrderMap.cartDTO.product_img_name}";
...
$(document).ready(function() {
  var phone = "${directOrderMap.memberDTO.phone}";
  var phoneHypen = phone.replace(/(^02.{0}|^01.{1}|[0-9]{3})([0-9]+)([0-9]{4})/,"$1-$2-$3");
